...
public class ScheduleDatabaseTest {
  ...
  public void setup() {
    ...
    synchronized (ScheduleDatabase.class) {
      db =
          Room.inMemoryDatabaseBuilder(context, ScheduleDatabase.class)
  ...
  private List<ScheduleRow> getData(String filename) {
    InputStream inputStream =
        ScheduleDatabaseTest.class.getClassLoader().getResourceAsStream(filename);
  ...
  private void testSchedule(String filename, @ServiceType int type) {
    ...
    for (int i = schedules.size() - 1; i >= 0; i--) {
      ...
      for (int j = i - 1; j >= 0; j--) {
        ...
        // get results
        List<String> resultTimes =
            mapResults(db.getResultsTesting(fromName, toName, type, today, now));
